...
-- 🚀【出库核销多轨引擎：修复报错字段，精准利用标识码作为全系统主键】
OUTER APPLY (
    SELECT 
    ...
    FROM (
        ...
        WHERE PX._XORN = 'X' 
          ...
          AND (
              (ISNULL(HX.DINH_DANH_HANG_HOA, '') <> '' AND HX.DINH_DANH_HANG_HOA = H.DINH_DANH_HANG_HOA)
              OR (HX.SO_PHIEU_N = P.SO_PHIEU)
              OR (HX.SOTK_N = P.SOTK AND HX.STTHANG_N = H.STTHANG AND P.SOTK IS NOT NULL AND P.SOTK <> '')
          )

        UNION ALL

        -- 2. 报废销毁 (Tiêu hủy - 仅使用合法字段)
        ...
        WHERE TH.MA_KNQ = 'VNNSL' 
          AND TH.TRANG_THAI = 1
          AND TH_CT.MA_SP = H.MA_SP
          AND (
              (ISNULL(TH_CT.DINH_DANH_HANG_HOA, '') <> '' AND TH_CT.DINH_DANH_HANG_HOA = H.DINH_DANH_HANG_HOA)
              OR (TH_CT.SO_PHIEU_N = P.SO_PHIEU)
          )

        UNION ALL

        -- 3. 产权转移 (Chuyển quyền - SOTK在主表VB中)
        ...
        WHERE VB.TRANG_THAI = '2'
          AND VB_H.MA_SP = H.MA_SP
          AND (
              (ISNULL(VB_H.DINH_DANH_HANG_HOA, '') <> '' AND VB_H.DINH_DANH_HANG_HOA = H.DINH_DANH_HANG_HOA)
              OR (VB_H.SO_PHIEU_N = P.SO_PHIEU)
              OR (LTRIM(RTRIM(REPLACE(VB.SOTK, '.0', ''))) = LTRIM(RTRIM(REPLACE(P.SOTK, '.0', ''))) AND VB_H.STTHANG_N = H.STTHANG AND P.SOTK IS NOT NULL AND P.SOTK <> '')
          )
    ) AS EXPORT_DATA
) X
...
WHERE 
    ...
    -- 结存量 > 0 (成功扣减了报废和产权转移后，这里将淘汰掉多出的 98 行)
