...
-- 🔻 请在这里修改你的查询参数 🔻
-- ========================================
DECLARE @MaKNQ NVARCHAR(50) = 'VNNSL';          -- ★ 必须修改：你的保税仓代码
DECLARE @TargetDate DATETIME = GETDATE();       -- 默认盘点截止到今天此刻
...
-- 【绝杀清零】：系统默认会扣减完没余额的货，直接物理删除！只留纯正的 OnHand 实存！
-- ★ 业务调整注意：如果你想看到“已经全部出清”的货物，把下面这行注释掉即可！
...
SELECT 
    ...
    SO_HD,                -- ★ [合规] 核心分组字段：保税合同号 (Số hợp đồng)
    ...
    SO_LUONG AS LUONG_NHAP, -- ★ [业务关注点 1]：总共进来了多少 (Lượng nhập)
    LUONG_XUAT,           -- ★ [业务关注点 2]：已经出库了多少 (Lượng xuất)
    LUONG_TON,            -- ★ [业务关注点 3]：当前还剩多少 (Lượng tồn)
    ...
    SO_NGAY_TON,          -- ★ [业务关注点 4]：该批货物实际滞库天数 (Số ngày tồn)
    GHI_CHU               -- [其他] 系统级动作备注 (Ghi chú)
FROM #NHAP2 
ORDER BY SO_HD ASC, NGAY_PHIEU ASC, SO_PHIEU ASC, STTHANG ASC;
