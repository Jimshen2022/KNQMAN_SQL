...
DECLARE @StartDate DATETIME = '2026-01-01';     --【变量：入库开始日期】
...
-- --------------------------------------------------------------------
-- STEP 3: 追加追加写入所有【普通散货 / 件货（Type = 2）】的正式生效入库明细
-- --------------------------------------------------------------------
INSERT INTO #NHAP 
SELECT 
    CAST(CAST(A.DHOPDONGID AS VARCHAR) + ';' + CAST(A.TYPE AS VARCHAR) + ';' + CAST(B.DINH_DANH_HANG_HOA AS VARCHAR) + ';' AS NVARCHAR(100)) AS CKEYS, 
...
INTO #DVANBAN 
FROM DVANBAN_HANG A, DVANBAN B, DHOPDONG F, DHOPDONG G
...
INTO #DVANBAN2 FROM #DVANBAN GROUP BY CKEYS;
